			...
			if(isShowOldDate){
				for(var indexP in orderTkviewStock){
					if(date== (orderTkviewStock[indexP].dateInfo).substring(0,10)){
						...
						if(isShowStock==true){
							result += "<input type='hidden' id='"+tempDate.getTime()+"_stock' value='"+orderTkviewStock[indexP].stockCount+"'/>";
							result += "<span  style='color:#ff6000;float:left;font-size:12px;'>余"+orderTkviewStock[indexP].stockCount+"</span>";
						}
				...
				if(date>=currerentDate){
					for(var indexP in orderTkviewStock){
						if(date == (orderTkviewStock[indexP].dateInfo).substring(0,10)){
							result += tempDate.getDate();
							if(orderTkviewStock[indexP].isEnabled==0){
								result +="<i style='width:8px;' src='iconfont icon-remove'></i>";
							}else{
								;
							}
							if(orderTkviewStock[indexP].isApply==1){
