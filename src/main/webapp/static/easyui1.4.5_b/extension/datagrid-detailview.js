		...
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					cc.push('<span class="datagrid-row-expander datagrid-row-expand iconfont icon-datagridRowExpand" style="display:inline-block;width:16px;height:16px;cursor:pointer;" />');
					//cc.push('</div>');
	...
	expandRow: function(jq, index){
		return jq.each(function(){
			...
			if (expander.hasClass('datagrid-row-expand')){
				expander.removeClass('datagrid-row-expand iconfont icon-datagridRowExpand').addClass('datagrid-row-collapse iconfont icon-datagridRowCollapse');
				var tr1 = opts.finder.getTr(this, index, 'body', 1).next();
				var tr2 = opts.finder.getTr(this, index, 'body', 2).next();
				tr1.show();
				tr2.show();
	...
	collapseRow: function(jq, index){
		return jq.each(function(){
			...
			if (expander.hasClass('datagrid-row-collapse')){
				expander.removeClass('datagrid-row-collapse iconfont icon-datagridRowCollapse').addClass('datagrid-row-expand iconfont icon-datagridRowExpand');
				var tr1 = opts.finder.getTr(this, index, 'body', 1).next();
				var tr2 = opts.finder.getTr(this, index, 'body', 2).next();
				tr1.hide();
				tr2.hide();
